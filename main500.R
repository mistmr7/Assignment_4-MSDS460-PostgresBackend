...
create_faker_data <- function(num_rows) {
  employee_data <- list()
  # Ensure unique 9-digit employee IDs by generating all at once
  employee_ids <- sample(100000000:999999999, size = num_rows, replace = FALSE)
  for (i in 1:num_rows) {
    # Sample one unique employee ID without replacement
    employee_id <- sample(employee_ids, size = 1, replace = FALSE)
    # Remove the selected employee ID from the list to avoid duplication
    employee_ids <- employee_ids[employee_ids != employee_id]
...
write.xlsx(df, "R_output500.xlsx", rowNames = FALSE)
